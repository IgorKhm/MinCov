...
class CovTreeHashTest:
    ...
    def _accelerate(self, current_node: CovNode):
        ...
        """
        ance_node = current_node.get_parent()

        while ance_node is not None:
            # self.num_of_comparisons += 1
            if all(current_node.marking >= ance_node.marking):
                ...
                if self.keep_accelerations:
                    ...
                    (pre, incidence) = self._compute_pre_and_incidence_acceleration(current_node, ance_node)
                    ...
                    current_node._tranFromParent = ance_node.get_transitions()
                    ...
                    acc = np.where(current_node.marking > ance_node.marking, (float("inf")), 0)
                    current_node.marking = current_node.marking + acc
                    self._verSet.add(tuple(current_node.marking))

                return ance_node

            ance_node = ance_node.get_parent()
    ...
    def _find_and_delete_smaller(self, current_node: CovNode, nodes_to_delete: [CovNode], accelerated):
        ...
        # num_of_vertices: int = len(self._vertices)
        current_marking = current_node.marking
        # index = self._vertices.index(current_node)  # saves checking every time that we are not deleting the currentNode
    ...
    def generate_cov_tree(self, root_mark=None):
        ...
        while clover_is_not_ready:
            while len(self._front) != 0:
                ...
                if self.timeout < time.time() - start_time:
                    return None
                current_node = None
                # i = np.random.random_integers(0, len(self._front) - 1)
                ...
                while 1:
                    ...
                    if tran.is_fireable_from(current_parent_node.marking):
                        # if is_fire(tran._pre_short, current_parent_node.marking):
                        new_marking = tran.apply_on_marking(current_parent_node.marking)
                        # new_marking = fire_tran(tran._incidence_short, current_parent_node.marking)
                ...
                if not current_node:
                    continue
                # b=0
                # for i in range(len(current_node.marking)):
                #     print(current_node.marking[i])
                ...
                # self._explore_successor(current_node)
                # Begin - Code for performance #
        ...
        print("average vertices size: %f" % self.average_vertics_size)
        # print("%d" % self.num_of_accelaration_tried)
    ...
    def check_for_cover(self, target: np.array, petri_file="", root_mark=None):
        ...
        start_time = time.time()
        print("loaded petri")
        solver, variables = build_cpn_solver(qc_petrinet, None, qc_targets, domain='N')
        ...
        def coverable(markings):
            return igor_check_cpn_coverability_z3(solver, initial_vars, markings)
