...
import z3
from qcover.petri import load_petrinet, constraint_vector, petrinet_lossy
from qcover.cpn import reachable, build_cpn_solver
from qcover.solver_utils import set_markings_constrained
from qcover.upward_sets import update_upward, in_upward, merge_upward
...
def igor_check_cpn_coverability_z3(solver, target_vars, markings):
    solver.push()
    set_markings_constrained(solver, target_vars, [[('=', p) for p in m]
                                                   for m in markings])
    result = solver.check()
...
def coverability(petrinet, init, targets, prune=False, max_iter=None):
    ...
    solver, variables = build_cpn_solver(petrinet, init, targets=None,
                                         domain='N')
    solver.set("timeout", 900000)
    _, _, target_vars = variables
    print("time %3f" % (time() - start_time))
    def coverable(markings):
        if time() - start_time > timeout:
            raise Exception("timeout")
        ret = check_cpn_coverability_z3(solver, target_vars, markings)  # one from the prebaisis
        if time() - start_time > timeout:
            raise Exception("timeout")
        return ret
